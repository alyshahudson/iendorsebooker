...
const prepareFontLoad = (fontList) =>
	Promise.all(fontList.map((font) => document.fonts.load(font)));
...
async function startGeneratingImage() {
	const fontList = [
		'700 60px Jubilat',
		'700 italic 60px Jubilat',
		'700 60px freight-sans-pro',
		'500 60px freight-sans-pro',
	];
...
function imagesAreNowLoaded() {
	...
	if (background.includes('-')) {
		ctx.font =
			'500 ' + String((20 * h) / 1000) + 'px freight-sans-pro, monospace';
		ctx.textAlign = 'right';
		...
		ctx.fillText(link, w * 0.99, h * 0.02, w * 0.9);
	}
	...
	ctx.font =
		'700 ' + String((30 * h) / 1000) + 'px freight-sans-pro, monospace';
	ctx.textAlign = 'left';
	...
	ctx.fillText(
		'“',
		w * 0.031,
		endorseeName != 'Bernie' ? h * 0.77 : h * 0.75,
		w * 0.9
	);
	...
	for (size = 50; size > 15; size -= 3) {
		y = origY;
		ctx.font =
			'700 ' + String((size * h) / 1000) + 'px freight-sans-pro, monospace';
	...
	var IM_VOLUNTEERING =
		endorseeName == 'Bernie'
			? 'I’m volunteering for Bernie by:'
			: 'I’m volunteering for Bernie & ' + endorseeName + ' by:';
	...
	if (volunteer) {
		ctx.font =
			'500 ' + String((volSize * h) / 1000) + 'px freight-sans-pro, monospace';
		...
		ctx.font =
			'500 ' +
			String((volSize * 0.75 * h) / 1000) +
			'px freight-sans-pro, monospace';
	...
	ctx.font =
		'700 ' + String((20 * h) / 1000) + 'px freight-sans-pro, monospace';
	ctx.fillText(GENERATED_HASHTAG, w * 0.051 + thatWidth, h * 0.96, w * 0.9);

	ctx.font =
		'500 ' + String((15 * h) / 1000) + 'px freight-sans-pro, monospace';
	ctx.fillStyle = 'RGBA(255,255,255,0.4';
	ctx.fillText(
		'Not affiliated with the Bernie 2020 campaign' +
			(endorseeName != 'Bernie' ? ' or other campaigns' : ''),
		w * 0.051,
		h * 0.98,
		w * 0.9
	);
